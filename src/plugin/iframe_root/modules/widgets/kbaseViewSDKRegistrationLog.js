...
define(['jquery', 'kb_service/client/catalog', 'kb_widget/legacy/authenticatedWidget'], function ($, Catalog) {
    ...
    $.KBWidget({
        ...
        init: function (options) {
            ...
            this.catalogClient = new Catalog(this.runtime.getConfig('services.catalog.url'), {
        ...
        render: function () {
            ...
            self.$log_window = $(
                '<textarea style="width:100%;font-family:Monaco,monospace;font-size:9pt;color:#555;resize:vertical;" rows="' +
                    self.options.n_rows +
                    '" readonly>'
            );
        ...
        updateBuildState: function (state, error, build_info) {
            ...
            if (state === 'error') {
                ...
                self.$registration_state_td.empty().append(
                    $('<span>')
        ...
        loading: function (isLoading) {
            if (isLoading) this.showMessage($('<i>').addClass('fa fa-spinner fa-2x fa-spin'));
