define([
    ...
    $.KBWidget({
        ...
        renderControlToolbar: function () {
            ...
            var $verB = $('<a>')
                .append('Beta Versions')
                .on('click', function () {
                    self.switchVersion('beta');
                });
            var $verD = $('<a>')
                .append('Development Versions')
                .on('click', function () {
                    self.switchVersion('dev');
                });
        ...
        refreshLocalFunctionList: function (tag) {
            ...
            return self.catalog
                .list_local_functions(localFunctionSelection)
        ...
        renderFunctionList: function () {
            ...
            if (self.tag) {
                ...
                if (self.tag === 'dev') {
                    ...
                    self.$functionListPanel.append(
                        $('<div>')
                            .css(text_css)
                            .append('Currently viewing Functions in beta.')
                    );
                }
            }
            ...
            if (organizeBy === 'name_az') {
                ...
                var types = [];
                for (const k in self.inputTypes) {
                    types.push(k);
                }
                types.sort();
                ...
                for (let k = 0; k < self.functionList.length; k++) {
                    ...
                    if (self.functionList[k].info.tags.input.kb_types.length > 0) {
                        const input_types = self.functionList[k].info.tags.input.kb_types;
                        for (let i = 0; i < input_types.length; i++) {
                            $typeDivLookup['KBase Type: ' + input_types[i]].append(
                                self.functionList[k].getNewCardDiv()
                            );
                        }
                    }
                }
                ...
                for (let k = 0; k < self.functionList.length; k++) {
                    ...
                    if (self.functionList[k].info.tags.output.kb_types.length > 0) {
                        const output_types = self.functionList[k].info.tags.output.kb_types;
                        for (let i = 0; i < output_types.length; i++) {
                            $typeDivLookup['KBase Type: ' + output_types[i]].append(
                                self.functionList[k].getNewCardDiv()
                            );
                        }
                    }
