define([
    ...
    $.KBWidget({
        ...
        init: function (options) {
            ...
            if (options.namespace) {
                if (options.namespace === 'l.m') {
                    ...
                    self.$elem.append(
                        '&nbsp Legacy apps not supported on this page yet.  Go here for now:' +
                            '<a href="/#narrativestore/app/' +
                            options.id +
                            '" target="_parent">' +
                            options.id +
                            '</a>'
        ...
        getAppFullInfo: function () {
            ...
            return self.nms.get_method_full_info(params).then(function (info_list) {
        ...
        getAppSpec: function () {
            ...
            return self.nms.get_method_spec(params).then(function (specs) {
        ...
        getModuleInfo: function () {
            ...
            return self.catalog
                .get_module_version(moduleSelection)
                .then(function (info) {
                    if (!self.tag) {
        ...
        renderInfo: function () {
            ...
            if (self.isLegacyMethod) {
                return self.nms.status().then(function (status) {
                    ...
                    self.$infoPanel.append(
                        $('<table>')
                            .css({ border: '1px solid #bbb', margin: '10px', padding: '10px' })
                            /*.append($('<tr>')
                            ...
                            .append(
                                $('<tr>')
                                    .append(
                                        $('<th style = "vertical-align : top; padding-right : 5px">').append(
                                            'Yaml/Spec Location '
                                        )
                                    )
                ...
                if (self.moduleDetails.info) {
                    ...
                    if (self.isGithub) {
                        ...
                        self.$infoPanel.append(
                            $('<table>')
                                .css({ border: '1px solid #bbb', margin: '10px' })
                                .append(
                                    $('<tr>')
                                        .append(
                                            $('<th style = "vertical-align : top;" >')
                                                .css('padding', p)
                                                .append('App Specification: ')
                                        )
                                        .append(
                                            $('<td>')
                                                .css('padding', p)
                                ...
                                .append(
                                    $('<tr>')
                                        .append(
                                            $('<th style = "vertical-align : top;" >')
                                                .css('padding', p)
                                                .append('Module Commit:  ')
                                        )
                                        .append(
                                            $('<td>')
                                                .css('padding', p)
                                                .append(self.moduleDetails.info.git_commit_hash)
                                        )
                                )
                        );
                    } else {
                                    ...
                                    $('<tr>')
                                        ...
                                        .append(
                                            $('<td>')
                                                ...
                                                .append(
                                                    '<a href="' +
                                                        self.moduleDetails.info.git_url +
                                                        '" target="_blank">' +
                                                        self.moduleDetails.info.git_url +
                                                        '</a>'
                                                )
        ...
        updateFavoritesCounts: function () {
            ...
            return self.catalog
                .list_app_favorites(list_app_favorites)
                .then(function (users) {
        ...
        updateMyFavorites: function () {
            ...
            if (self.runtime.service('session').isLoggedIn()) {
                return self.catalog
                    .list_favorites(self.runtime.service('session').getUsername())
                ...
                .then(function (stats) {
                    ...
                    if (stats.length > 0) {
                        ...
                        */
                        ...
                        self.$headerPanel.find('.kbcb-runs').append(
                            $('<div>')
                                .addClass('container-fluid')
                                .append($row)
                        );
                        ...
                        $row.append(
                            $('<div>')
                                ...
                                .append(
                                    $('<span>')
                                        .addClass('kbcb-run-count')
                                        .append(successPercent.toPrecision(3) + '%')
                                )
                                .tooltip({
                                    title:
                                        'Ran sucessfully without error ' +
                                        successPercent.toPrecision(4) +
                                        '% of the time (' +
                                        goodCalls +
                                        '/' +
                                        s.number_of_calls +
        ...
        renderMethod: function () {
            ...
            if (m.icon && self.nms_base_url) {
                if (m.icon.url) {
                    $logoSpan.html(
                        $('<img src="' + self.nms_base_url + m.icon.url + '">').css({
                            'max-width': '85%',
            ...
            $titleSpan.append(
                $('<div>')
                    .addClass('kbcb-app-page-title')
                    .append(m.name)
                    .append($('<span>').css({ 'font-size': '0.5em', 'margin-left': '0.5em' }))
            );
            ...
            if (m.authors.length > 0) {
                var $authorDiv = $('<div>')
                    .addClass('kbcb-app-page-authors')
                    .append('by ');
                for (var k = 0; k < m.authors.length; k++) {
                    ...
                    $authorDiv.append(
                        $('<a href="/#people/' + m.authors[k] + '" target="_parent">')
                            .append(m.authors[k])
            ...
            if (self.isLegacyMethod || self.isLegacyApp) {
                $nRuns
                    .append('<small>Run statistics cannot be displayed for this method.</small>')
                    .css('text-align', 'left');
            }

            $header.append(
                $('<div>')
                    .addClass('kbcb-app-page-stats-bar container')
                    .append(
                        $('<div>')
                            .addClass('row')
                            .append($starDiv)
            ...
            if (spec.parameters && spec.parameters.length) {
                ...
                $.each(spec.parameters, function (idx, param) {
                ...
                $.each(['inputs', 'outputs', 'parameters'], function (idx, ui_class) {
                    ...
                    $.each(sortedParams[ui_class], function (idx, param) {
                        ...
                        $li.append(
                            $.jqElem('ul')
                                ...
                                .append(
                                    $.jqElem('li')
                                        .append($.jqElem('b').append(param.ui_name))
                                        .append(types)
                                        .append(
                                            $.jqElem('ul')
                                                .css('list-style-type', 'none')
            ...
            if (spec.fixed_parameters && spec.fixed_parameters.length) {
                $parametersDiv = $.jqElem('div').append($.jqElem('h4').append('Fixed parameters'));
                ...
                $.each(spec.fixed_parameters, function (idx, param) {
                    var $li = $.jqElem('li'); //.append('Parameter ' + (idx + 1)));
                    $li.append(
                        $.jqElem('ul')
                            .css('list-style-type', 'none')
            ...
            if (m.publications && m.publications.length) {
                var $pubsDiv = $.jqElem('div').append($.jqElem('strong').append('Related publications'));
                var $publications = $.jqElem('ul');
                $.each(m.publications, function (idx, pub) {
                    $publications.append(
                        $.jqElem('li')
            ...
            if (m.kb_contributors !== undefined && m.kb_contributors.length) {
                $.each(m.publications, function (idx, name) {
                    $publications.append($.jqElem('li').append(name));
                });
