    ...
    $.KBWidget({
        ...
        renderBasicStatus: function () {
            ...
            self.$basicStatusDiv.append(
                '<a href="' +
                    self.runtime.getConfig('services.catalog.url') +
                    '">' +
                    self.runtime.getConfig('services.catalog.url') +
                    '</a>'
            );
        ...
        rerenderPendingRelease: function () {
            ...
            self.getPendingReleases().then(function () {
                self.renderPendingRelease();
            });
        },
        ...
        renderModuleList: function () {
            ...
            var $modNameAct = $('<input type="text" size="50">')
            ...
            $activate.on('click', function () {
                ...
                $actResult.empty();
                self.catalog
                    .set_to_active({ module_name: modName })
                    .then(function () {
                        $actResult.prepend(
                            $('<div role=alert>')
                                .addClass('alert alert-success')
                                .append(
                                    '<b>Success.</b> The module ' +
                                        modName +
                                        ' was set to Active. Refresh this page to update the list.'
                                )
                        );
                    })
                    .catch(function (err) {
            ...
            $deactivate.on('click', function () {
                ...
                $actResult.empty();
                self.catalog
                    .set_to_inactive({ module_name: modName })
                    .then(function () {
                        $actResult.prepend(
                            $('<div role=alert>')
                                .addClass('alert alert-success')
                                .append(
                                    '<b>Success.</b> The module ' +
                                        modName +
                                        ' was set to Inactive. Refresh this page to update the list.'
                                )
                        );
                    })
                    .catch(function (err) {
            ...
            // Module Deletion
            var $modNameDel = $('<input type="text" size="50">')
                .addClass('form-control')
                .css('margin', '4px');
            var $trash = $('<button>')
                .addClass('btn btn-default')
            ...
            $deleteModule
                .append($('<b>').append('Delete Module:'))
                .append(' (only allowed if not yet released):')
                .append('<br>');
            $deleteModule
                .append(
                    $('<div>')
            ...
            $trash.on('click', function () {
                ...
                var $confirm = $('<button>')
                    ...
                    .append('Confirm deletion of ' + modName + '?');
                ...
                $confirm.on('click', function () {
                    $confirm.hide();
                    self.catalog
                        .delete_module({ module_name: modName })
                        .then(function () {
                            $result.prepend(
                                $('<div role=alert>')
                                    .addClass('alert alert-success')
                                    .append(
                                        '<b>Success.</b> The module ' +
                                            modName +
                                            ' was deleted. Refresh this page to update the list.'
                                    )
                            );
                        })
                        .catch(function (err) {
            ...
            var k;
            for (k = 0; k < self.released_modules.length; k++) {
                $tbl.append(
                    $('<tr>')
                        .append(
                            $('<td>').append(
            ...
            $tbl = $('<table>').addClass('table table-hover table-condensed');
            for (k = 0; k < self.inactive_modules.length; k++) {
                $tbl.append(
                    $('<tr>')
                        .append(
                            $('<td>').append(
                ...
                for (var k = 0; k < self.requested_releases.length; k++) {
                    var addRow = function (mod) {
                        ...
                        $denyBtn.on('click', function () {
                            $approveBtn.prop('disabled', true);
                            $denyBtn.prop('disabled', true);
                            var $reasonInput = $('<input type="text" size="100">')
                                .addClass('form-control')
                                .css('margin', '4px');
                            var $submit = $('<button>')
                                .addClass('btn btn-danger')
        ...
        renderDevList: function () {
            ...
            var $devName = $('<input type="text" size="50">')
                .addClass('form-control')
                .css('margin', '4px');
            var $add = $('<button>')
                .addClass('btn btn-default')
        ...
        refreshClientGroups: function () {
            ...
            return self.getClientGroups().then(function () {
                self.renderClientGroupList();
            });
        },
        ...
        renderClientGroupList: function () {
            ...
            var $modName = $('<input type="text" size="50" placeholder="ModuleName">')
                .addClass('form-control')
                .css('margin', '4px');
            var $funcName = $('<input type="text" size="50" placeholder="function_name">')
                .addClass('form-control')
                .css('margin', '4px');
            var $groups = $('<input type="text" size="50" placeholder="group1,group2, ...">')
                .addClass('form-control')
                .css('margin', '4px');
            ...
            for (var k = 0; k < self.client_groups.length; k++) {
                    ...
                    (function (cg) {
                        return function () {
                            var confirm = window.confirm(
                                'Are you sure you want to remove this client group configuration?'
                            );
                            if (confirm == true) {
                                self.catalog
                ...
                $tbl.append(
                    $('<tr>')
                        ...
                        .append($('<td>').append(cliGroupString))
                        .append($('<td>').append($trash))
                );
            }
        ...
        refreshVolumeMounts: function () {
            ...
            return self.getVolumeMounts().then(function () {
                self.renderVolumeMountList();
            });
        },
        ...
        renderVolumeMountList: function () {
            ...
            if (!self.isAdmin) {
                self.$volumeMountList.append(
                    $('<div>')
                        .css('margin', '1em')
                        .append('Only Admins can view volume mounts.')
                );
                return;
            }
            ...
            var $moduleName = $('<input type="text" size="50" placeholder="ModuleName">')
                .addClass('form-control')
                .css('margin', '4px');
            var $functionName = $('<input type="text" size="50" placeholder="function_name">')
                .addClass('form-control')
                .css('margin', '4px');
            var $clientGroup = $('<input type="text" size="50" placeholder="client_group_name">')
                .addClass('form-control')
                .css('margin', '4px');
            ...
            for (var x = 0; x < nOptions; x++) {
                volumeMountEntry.push({
                    $host_dir: $('<input type="text" size="50" placeholder="/my/host/path/' + (x + 1) + '">')
                        .addClass('form-control')
                        .css('margin', '4px'),
                    $con_dir: $('<input type="text" size="50" placeholder="/my/container/path/' + (x + 1) + '">')
                        .addClass('form-control')
                        .css('margin', '4px'),
                    $rw: $('<select>')
                        .addClass('form-control')
            ...
            $modifyVolMount
                .append($('<b>').append('Add / Modify Volume Mounts:'))
                .append(' (use the API if you need more than ' + nOptions + ' mounts)')
                .append('<br>');

            var $volMountInfo = $('<div>');
            for (x = 0; x < volumeMountEntry.length; x++) {
                ...
                $d.append(
                    $('<div>')
                        .addClass('col-md-3')
                        .append(volumeMountEntry[x]['$host_dir'])
                );
                $d.append(
                    $('<div>')
                        .addClass('col-md-3')
                        .append(volumeMountEntry[x]['$con_dir'])
                );
                $d.append(
                    $('<div>')
                        .addClass('col-md-2')
                        .append(volumeMountEntry[x]['$rw'])
                );
            ...
            $modify.on('click', function () {
                ...
                if ($moduleName.val()) {
                    config['module_name'] = $moduleName.val();
                }
                if ($functionName.val()) {
                    config['function_name'] = $functionName.val();
                }
                if ($clientGroup.val()) {
                    config['client_group'] = $clientGroup.val();
                }
            ...
            $tbl.append(
                $('<tr>')
                    ...
                    .append(
                        $('<th>').append(
                            '<b>Host Directory &nbsp;&nbsp;<i class="fa fa-arrow-right"></i>&nbsp;&nbsp; Container Directory</b>'
                        )
                    )
                    .append($('<th>'))
            );
            ...
            for (var k = 0; k < self.volume_mounts.length; k++) {
                ...
                $tbl.append(
                    $('<tr>')
                        ...
                        .append($('<td>').append(client_group))
                        .append($('<td>').append(volMountStr))
                        .append($('<td>').append($trash))
                );
            }
        ...
        renderSecureParamsControls: function () {
            var self = this;
            if (!self.isAdmin) {
                self.$secureParams.append(
                    $('<div>')
                        .css('margin', '1em')
                        .append('Only Admins can manage secure parameters.')
                );
                return;
            }
                ...
                self.catalog
                    ...
                    .then(function (params) {
                        if (!params || params.length === 0) {
                            ...
                            $paramViewArea.append(
                                'No secure parameters found for module "' +
                                    currentModule +
                                    '" with version "' +
                                    viewVer +
                                    '"'
                        ...
                        var $table = $('<table class="table table-hover table-condensed">').append(
                        ...
                        params.forEach(function (param) {
                            ...
                            $delButton.click(function () {
                                var confirmStr =
                                    'REALLY delete secure parameter "' +
                                    param.param_name +
                                    '" from module "' +
                                    currentModule +
            ...
            $moduleSelect.on('change', function () {
                ...
                if (module.beta) {
                    $versionSelect.append(
                        verOption(module.beta.git_commit_hash, 'beta - ' + module.beta.git_commit_hash)
                    );
                }
                if (module.release) {
                    $versionSelect.append(
                        verOption(module.release.git_commit_hash, 'release - ' + module.release.git_commit_hash)
                    );
                }
                if (module.release_version_list && module.release_version_list.length > 1) {
                    for (var i = module.release_version_list.length - 2; i >= 0; i--) {
                        $versionSelect.append(
                            verOption(
                                module.release_version_list[i].git_commit_hash,
                                'old release - ' + module.release_version_list[i].git_commit_hash
                            )
                        );
                    }
                }
            ...
            $paramSetDiv.find('#pwcheck').on('change', function () {
            ...
            self.$secureParams
                .append(
                    $('<div class="row">')
                        ...
                        .append($('<div class="col-md-11">').append($moduleSelect))
                )
                .append(
                    $('<div class="row">')
                        .append($('<div class="col-md-1"><b>Version</b></div>'))
                        .append($('<div class="col-md-11">').append($versionSelect))
                )
                .append(
                    $('<div class="row">')
                        .append($('<div class="col-md-1"><b>Parameters</b></div>'))
                        .append($('<div class="col-md-11">').append($paramViewArea))
                )
                .append($paramSetDiv);
        },
