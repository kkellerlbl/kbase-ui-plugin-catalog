    ...
    $.KBWidget({
        ...
        init: function (options) {
            ...
            Promise.all(loadingCalls)
                .then(function () {
                    self.render();
                    self.hideLoading();
                    return self.getAppInfo().then(function () {
                        var p = Promise.all([
        ...
        render: function () {
            ...
            $header
                ...
                .append(
                    $('<span>')
                        .css({ 'font-weight': 'bold', 'font-size': '1.2em', 'margin-left': '0.5em' })
                        .append(verStrHeader)
                );

            $header.append($('<h4>').append('<a href="' + info.git_url + '" target="_blank">' + info.git_url + '<a>'));
            ...
            if (info.release) {
                $versionDiv.append(
                    self.renderCollapsableVersionDiv(
            ...
            if (info.beta) {
                $versionDiv.append(
                    self.renderCollapsableVersionDiv('Beta Version', self.renderVersion('beta', info.beta))
                );
            } else {
            ...
            if (versions) {
                if (versions.length > 0) {
                    ...
                    $versionDiv.append(self.renderCollapsableVersionDiv('Old Releases', $oldReleaseDiv));
                }
            }
        },
        ...
        renderModuleAdminDiv: function () {
            ...
            $adminContent.append(
                $('<div>')
                    .css({ width: '500px', 'margin-left': '2em' })
                    .append($stateTable)
            );
            var width = '15%';
                ...
                $('<div>')
                    ...
                    .append(
                        $('<button>')
                            ...
                            .append('Migrate Current Dev Version to Beta')
                            .on('click', function () {
                                self.catalog
                                    .push_dev_to_beta({ module_name: self.module_name })
                                    .then(function () {
                                        $manageStatusPanel.prepend(
                                            $('<div role=alert>')
                    ...
                    .append(
                        $('<button>')
                            .addClass('btn btn-default')
                            .append('Request New Release')
                            .on('click', function () {
                                self.catalog
                                    .request_release({ module_name: self.module_name })
                                    .then(function () {
                                        $manageStatusPanel.prepend(
                                            $('<div role=alert>')
                                                .addClass('alert alert-success')
        ...
        getModuleInfo: function () {
            ...
            return self.catalog
                .get_module_info(moduleSelection)
                .then(function (info) {
                    /*typedef structure {
                        string module_name;
                        string git_url;
        ...
        getModuleVersions: function () {
            ...
            return self.catalog
                .list_released_module_versions(moduleSelection)
        ...
        getModuleStatus: function () {
            ...
            return self.catalog
                .get_module_state(moduleSelection)
                .then(function (state) {
                    /*typedef structure {
                        boolean active;
                        boolean released;
