define([
    ...
    $.KBWidget({
        ...
        init: function (options) {
            ...
            self.getFunctionInfo().then(function () {
                self.hideLoading();
        ...
        getFunctionInfo: function () {
            ...
            return self.catalog
                .get_local_function_details({ functions: [params] })
                .then(function (info_list) {
                    ...
                    if (self.functionInfo) {
                        self.runtime.send(
                            'ui',
                            'setTitle',
                            [self.functionInfo.info.name, 'Function Catalog'].join(' | ')
                        );
                    }
                    return self.getModuleInfo(
                        self.functionInfo.info.module_name,
                        self.functionInfo.info.git_commit_hash
                    );
                })
                .catch(function (err) {
                    console.error('ERROR');
        ...
        renderInfo: function () {
            ...
            $titleSpan.append(
                $('<div>')
                    ...
                    .append(
                        $('<span>')
                            .css({ 'font-size': '0.5em', 'margin-left': '0.5em' })
            ...
            if (self.module_version) {
                ...
                if (git_url.indexOf('github.com') > -1) {
                    $gitDiv.append(
                        '<b>Github Source Commit:</b>&nbsp; &nbsp; <a href="' +
                            git_url +
                            '/tree/' +
                            self.functionInfo.info.git_commit_hash +
                            '" target="_blank">' +
                    ...
                    var specUrl =
                        ...
                        self.functionInfo.info.git_commit_hash +
                        '/ui/local_functions/' +
                        self.functionInfo.info.function_id +
                        '.json';
                    $gitDiv.append(
                        '<b>Github Documentation Spec:</b>&nbsp; &nbsp; <a href="' +
                            specUrl +
                            '" target="_blank">' +
                            specUrl +
                            '</a><br>'
                    ...
                    if (self.module_version.compilation_report) {
                        if (
                            self.module_version.compilation_report.function_places &&
            ...
            if (self.functionInfo.info.kidl) {
                if (self.functionInfo.info.kidl.parse) {
                    ...
                    var code = $('<div>')
                        ...
                        .css({ margin: '5px', 'font-size': '1.1em' });
                            ...
                            if (parse.input[i].comment) {
                                ...
                                input_comments
                                    ...
                                    .append(
                                        $('<div>')
                                            .css({ 'margin-left': '1em' })
                                            .append(self.escapeHtml(parse.input[i].comment))
                                    );
                            }
                        }
                    }
                            ...
                            if (parse.output[i].comment) {
                                ...
                                output_comments
                                    ...
                                    .append(
                                        $('<div>')
                                            .css({ 'margin-left': '1em' })
                                            .append(self.escapeHtml(parse.output[i].comment))
                                    );
                            }
                        }
                    }
